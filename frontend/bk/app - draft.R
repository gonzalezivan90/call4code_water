...
server <- function(input, output, session) {
  ...
  isolate(observeEvent(input$go_find,{
    ...
    output$findLeaf <- renderLeaflet({
      ...
      #landaoi <- land$first()$select('water')$clip(basaoi)
      #basaoi_gi <- basaoi$getInfo()
      ...
      # mapBas <- Map$addLayer(basaoi, opacity = .5,
      #                        visParams =  list(color = "9099A0"),
      #                        name = "Basin")
      # mapRiv <- Map$addLayer(rivWWF, 
      #                        visParams =  list(color = "3181BF"),
      #                        name = "River")
      # mapOrig <- Map$addLayer(ee$Feature(aoiPoint),
      #              visParams =  list(color = "FF0000"),
      #              name = "Origin")
      # mapDest <- Map$addLayer(dstPoint$draw(color = "006600", strokeWidth = 5), 
      #                          {}, 
      #                          name = "Destination")
      # leafGee <- Reduce('+',  list(mapBas,
      #                        mapRiv, mapOrig, mapDest) )
                        
      #save(inClick,leafGee, dstPoint, snpDf, aoiPoint, rivWWF, basaoi, file = 'templeaf.RData'); 
      #load(file = 'templeaf.RData')
      
      leafGee 
      # %>% addLegend("bottomleft",
      #                       colors = c("#9099A0", "#3181BF", "#FF0000", "#006600"),
      #                       labels= c('Basin', 'Rivers', 'Source', 'Destination') # , title= "Ecosystem"
      #                       )
    })
